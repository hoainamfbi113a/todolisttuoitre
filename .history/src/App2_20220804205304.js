  ...
  return (
    <div className="App">
      <div className="wrapper">
        ...
        <main>
          <div className="add-section">
            <Form className='form' form={context.form}>
              ...
              <Form.Item>
                <Select defaultValue={context.level} style={{minWidth: '100px'}} onChange={item => context.handleChangeLevel(item)}>
          ...
          <div className="search-section">
              ...
              <Input
                ...
                onChange={e => {
                  if (e.target.value === '') {
                    ...
                    const qualifiedTasks = context.tasks.filter(task => task.task.includes(e.target.value));
            ...
            <Button 
              shape='circle' 
              icon={context.isSearching ? <CloseOutlined/> :<SearchOutlined/>}
        ...
        <Table
          ...
          dataSource={context.searchTasks.length === 0 ? context.tasks : context.searchTasks} 
        ...
        <footer className="footer">
          <Progress 
            type='circle' 
            percent={context.tasks.filter(task => task.completed).length / context.tasks.length * 100} 
            format={() => {
          ...
          <div className='buttons'>
            <Button
              ...
              onClick={() => {
                const alreadyCheckedAll = context.tasks.every(task => task.completed === true)
            ...
            </Button>
            {
              ...
              <Button 
                ...
                type='danger'
                onClick={() => {
                  const doneTasks = context.tasks.filter(task => task.completed === true)
                  doneTasks.forEach(doneTask => context.deleteTodo(doneTask.id))
                  context.setTasks(prevState => {
      ...
      >
        ...
        />
        <Select 
          value={context.editingTaskLevel === 1 ? 'Easy' : context.editingTaskLevel === 2 ? 'Medium' : 'Hard'} 
          style={{minWidth: '100px'}} 
          onChange={item => context.setEditingTaskLevel(item === 'Easy' ? 1 : item === 'Medium' ? 2 : 3 ) }
        >
            {context.levels.map((item, index) => {
              return <Select.Option key={index} value={item}>
                      {item}
